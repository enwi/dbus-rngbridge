...
class DbusShelly3emService:
    def __init__(self, paths, productname='RNGBridge', connection='RNGBridge HTTP JSON service'):
    ...
    def _getShellyStatusUrl(self):
        ...
        if accessType == 'OnPremise':
            ...
            raise ValueError("AccessType %s is not supported" %
                             (config['DEFAULT']['AccessType']))
    ...
    def _update(self):
        try:
            ...
            self._dbusservice['/Yield/Power'] = state['p']['vo'] * \
                state['p']['cu']
            ...
            # logging
            logging.debug("PV Power (/Yield/Power): %s" %
                          (self._dbusservice['/Yield/Power']))
            logging.debug("Battery Voltage (/Dc/0/Voltage): %s" %
                          (self._dbusservice['/Dc/0/Voltage']))
            logging.debug("Battery Current (/Dc/0/Current): %s" %
            ...
            self._dbusservice['/UpdateIndex'] = (
                self._dbusservice['/UpdateIndex'] + 1) % 256

            # update lastupdate vars
            self._lastUpdate = time.time()
        except (ValueError, requests.exceptions.ConnectionError, requests.exceptions.Timeout, ConnectionError) as e:
            logging.critical(
                'Error getting data from RNGBridge - check network or RNGBridge status. Setting power values to 0. Details: %s', e, exc_info=e)
